...
defmodule Ex3 do
  ...
  def tamanho_strings(ls), do: map(ls, &String.length/1)
  ...
  def converte_para_inteiros(l), do: map(l, &String.to_integer/1)
  ...
  def soma_100_lista(l), do: map(l, fn (v) -> v + 100 end)
  ...
  def soma_n_lista(l, n), do: map(l, fn (v) -> v + n end)
  ...
  """
  def adiciona_prefixo([], pre), do: []
  def adiciona_prefixo(l, pre), do: map(l, fn (v) -> "#{pre}#{v}" end)
  ...
  """
  def adiciona_sufixo([], sufixo), do: []
  def adiciona_sufixo(l, sufixo), do: map(l, fn (v) -> "#{v}#{sufixo}" end)
  ...
  def and_lista([false]), do: false
  def and_lista([true]), do: true
  ...
  def or_lista([false]), do: false
  def or_lista([true]), do: true
  ...
  930
  """
  def fold_left([x | r], val, f), do: fold_left(r, f.(val, x), f)
  def fold_left([], val, _), do: val
  ...
  def concatena_listas(ll), do: fold_right(ll, [], fn x, y -> x ++ y end)
  ...
  """
  def concat_strings_sep([], sep), do: ""
  def concat_strings_sep([head | tail], sep) do
    if length(tail) >= 1 do
      "#{head}#{sep}#{Enum.at(tail, 0)}#{sep}#{concat_strings_sep(List.delete_at(tail, 0), sep)}"
    else
      "#{head}"
    end
  end
  ...
  ["euforia", "Moria"]
  """
  def strings_que_contem(ls, meio), do: filter(ls, fn atual -> String.contains?(atual, meio) end)
  ...
  [1, 3, 5]
  """
  def par_3n_mais_1(l), do: filter(l, fn atual -> Integer.is_even((atual*3) + 1) end)
end
